...
def render_header():
    ...
    st.caption("Enter ASIN to get product insights.")
...
def render_product_card(product):
    with st.container(border=True):
        ...
        try:
            ...
            cols[0].write("Error loading the image")

        with cols[1]:
            st.subheader(product.get("title") or product.get("asin"))
            ...
            if st.button(
                "Start analyzing competitors", key=f"analyze_{product['asin']}"
            ):
...
def main():
    st.set_page_config(
        page_title="Amazon Competitor Analysis", page_icon="📃", layout="wide"
    )
    render_header()
    ...
    if st.button("Scrape product...") and asin:
        ...
        st.success("Product scraped succesfully!")
    ...
    if selected_asin:
        ...
        with col2:
            if st.button("Reresh competitors"):
